...
# orig shape [batch, 2, 64, 256]
# predicted  [batch, 2, 40, 256]                 0:5    45:64  from orginal lidar
...
original_loader    	  = torch.utils.data.DataLoader(dataset_original, batch_size=args.batch_size, shuffle=False, num_workers=4, drop_last=False)

dataset_predicted  	  = np.load(args.pred)
predicted_loader      = torch.utils.data.DataLoader(dataset_predicted, batch_size=args.batch_size, shuffle=False, num_workers=4, drop_last=False)
...
for  i,orig in enumerate(original_loader):
	...
	for index in trange(batch_size):
		...
		for k in range(reconSt.shape[1]):
			...
			for l in range(reconSt.shape[2]):
				if(withinRange(reconSt[0][k][l],dynamic[0][k+5][l])):
					...
					r.append(reconSt[0][k][l]-dynamic[0][k+5][l])  #size 1000
					z.append(reconSt[1][k][l]-dynamic[1][k+5][l])  #siez 1000
		...
		get_pcd_from_img_and_save(torch.Tensor(filtered.reshape([1,2,60,256])).cuda(),index+i*batch_size,'pcd/'+str(args.folder)+'/')
