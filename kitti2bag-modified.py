...
#        imu.orientation.x = q[0]-0.0159970859487
#        imu.orientation.y = q[1]-0.0209919837212
#        imu.orientation.z = q[2]-0.670137561204
#        imu.orientation.w = q[3]-0.741767536985
...
def save_camera_data(bag, kitti_type, kitti, util, bridge, camera, camera_frame_id, topic, initial_time):
    ...
    if kitti_type.find("raw") != -1:
        ...
        image_path = os.path.join(image_dir, 'data1')
...
def getint(frame):
    return int(frame.split('.')[0])


def save_velo_data(bag, kitti, velo_frame_id, topic, folder):
    print("Exporting velodyne data")
    velo_path = os.path.join(kitti.data_path, 'velodyne_points')
    velo_data_dir = os.path.join(velo_path, str(folder))
    velo_filenames = sorted(os.listdir(velo_data_dir), key =getint)
...
#    bar = progressbar.ProgressBar()
    for dt, filename in iterable:
        ...
        # read binary data
        #this has to be changes for original vs model outout

        #model output
        scan = (np.fromfile(velo_filename, dtype=np.float32)).reshape(-1, 3)
        # exit(0)
...
def main():
    ...
    parser.add_argument("--name", type = str ,help = "sequence of the odometry dataset (between 00 - 21), option is only for ODOMETRY datasets.")
    parser.add_argument("--folder", type = str ,help = "sequence of the odometry dataset (between 00 - 21), option is only for ODOMETRY datasets.")
    ...
    if args.kitti_type.find("raw") != -1:
        ...
        bag = rosbag.Bag("{}_drive_{}_{}.bag".format(args.date, args.drive,  args.name), 'w', compression=compression)
     ...
     ###################################################################################################################################################
     # Do Not Export data I do not need
     ###################################################################################################################################################
     ...
     ####################################################################################################################################################
            save_velo_data(bag, kitti, velo_frame_id, velo_topic, args.folder)
