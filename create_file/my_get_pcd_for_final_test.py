...
def get_pcd_without_preprocess(file_,location):
    ...
    for frame_num in range(frames.shape[0]):
    	...
    	get_pcd_from_non_preprocessed_npy_and_save(torch.Tensor(frame.reshape([1,3,64,256])).cuda(),str(frame_num))
...
predicted_loader      = torch.utils.data.DataLoader(dataset_predicted, batch_size=dataset_original.shape[0], shuffle=False, num_workers=4, drop_last=False)
...
for  i,orig in enumerate(original_loader):
	...
	filtered_dy_recon = np.ndarray([batch_size,2,64,256])
	
	for index in trange(batch_size):
		dynamic  = original_temp[index:index+1].reshape([2,64,256])
		reconSt  = recons_temp[index:index+1].reshape([2,40,256])
		filtered = np.ndarray([2,64,256])
		...
		get_pcd_from_img_and_save(torch.Tensor(filtered.reshape([1,2,64,256])).cuda(),index+i*batch_size,'pcd/'+str(args.folder)+'/')
